...
function get_all_tasks_Overdue($conn){
	$sql = "SELECT * FROM tasks WHERE due_date < CURDATE() AND status != 'completed' AND (due_date IS NULL OR due_date != '0000-00-00') ORDER BY id DESC";
	$stmt = $conn->prepare($sql);
	$stmt->execute([]);
...
function count_tasks_Overdue($conn){
	$sql = "SELECT * FROM tasks WHERE due_date < CURDATE() AND status != 'completed' AND (due_date IS NULL OR due_date != '0000-00-00')";
	$stmt = $conn->prepare($sql);
	$stmt->execute([]);
...
function get_tasks_by_id($conn,$id){
...
function count_mytasks($conn, $id){
	$sql = "SELECT * FROM tasks WHERE assigned_to = ?";
	$stmt = $conn->prepare($sql);
	$stmt->execute([$id]);
...
function count_mytasks_Overdue($conn, $id){
	$sql = "SELECT * FROM tasks WHERE due_date < CURDATE() AND status != 'completed'  AND assigned_to = ? AND due_date != '0000-00-00'";
	$stmt = $conn->prepare($sql);
	$stmt->execute([$id]);
...
function count_mytasks_no_deadline($conn, $id){
...
function count_pending_mytasks($conn, $id){
...
function count_in_progress_mytasks($conn, $id){
	$sql = "SELECT * FROM tasks WHERE status = 'in_progress' AND assigned_to = ?";
	$stmt = $conn->prepare($sql);
	$stmt->execute([$id]);
...
function count_completed_mytasks($conn, $id){
